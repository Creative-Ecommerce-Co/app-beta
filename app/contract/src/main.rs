...
mod tests {
    ...
    fn test_update_product() {
        ...
        // Update the product
        product.price = 150;
        contract.update_product(user_id.clone(), product_id.clone(), product.clone());

        // Retrieve the updated product
        let updated_product = contract.get_product(user_id.clone(), product_id.clone()).unwrap();
    ...
    fn test_remove_product() {
        ...
        // Remove the product
        contract.remove_product(user_id.clone(), product_id.clone());

        // Ensure the product is removed
        assert!(contract.get_product(user_id.clone(), product_id.clone()).is_none());
    }
